...
void tratar_eventos_leds() {
    ...
    while (true) {
        ...
        if (!gpio_get(BOTOES[id1])) {
            ...
            if (id1 == 0 && index_neo < LED_COUNT) {  // BOTÃO A → incrementa
                ...
                if (quantidade > 0) {
            ...
            } else if (id1 == 2) {  // BOTÃO DO JOYSTICK → resetar fila
                 printf("Joystick pressionado - resetando fila...\n");
                  index_neo = 0;
                  inicio = 0;
                  fim = 0;
                 quantidade = 0;
                 ...
                 npClear();
                  ...
                  imprimir_fila();  // Mostra fila zerada no terminal
           }
